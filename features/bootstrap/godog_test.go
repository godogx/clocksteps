...
func registerClock(c *clocksteps.Clock, ctx *godog.ScenarioContext) {
...
func isNow(c *clocksteps.Clock) error {
	now := time.Now()
	ts := c.Now()
...
func isNotNow(c *clocksteps.Clock) error {
	now := time.Now()
	ts := c.Now()
	min := now.Add(-10 * time.Millisecond)
	max := now.Add(10 * time.Millisecond)
...
func expectTime(c *clocksteps.Clock, s string) error {
